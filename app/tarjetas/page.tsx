...
function CardMundialista({ data }: { data: PlayerData }) {
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden" style={{ aspectRatio: '3/4', background: 'linear-gradient(145deg, #6B1D3A 0%, #3D0A1F 55%, #0D0308 100%)', border: '3px solid #D4A843', boxShadow: '0 0 60px rgba(212,168,67,0.2), inset 0 1px 0 rgba(212,168,67,0.4)' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-2">
        {data.foto ? (
          <div className="w-28 h-28 rounded-full overflow-hidden" style={{ border: '3px solid #D4A843', boxShadow: '0 0 20px rgba(212,168,67,0.4)' }}>
            <img src={data.foto} alt="Foto" className="w-full h-full object-cover" style={{ transform: `scale(${data.fotoScale}) rotate(${data.fotoRotate}deg)`, transformOrigin: 'center' }} />
          </div>
        ) : (
          <div className="w-28 h-28 rounded-full flex items-center justify-center text-4xl font-black" style={{ background: 'rgba(212,168,67,0.12)', border: '3px solid rgba(212,168,67,0.4)', color: '#D4A843' }}>{nombre[0]?.toUpperCase() || '?'}</div>
        )}
      </div>
      <div className="flex justify-center mb-1"><div className="flex items-center gap-2"><div className="w-8 h-px bg-[#D4A843]/40" /><div className="w-5 h-5 rounded-full border border-[#D4A843]/50 flex items-center justify-center"><Trophy className="w-3 h-3 text-[#D4A843]" /></div><div className="w-8 h-px bg-[#D4A843]/40" /></div></div>
      <div className="px-4 py-4 text-center" style={{ background: 'linear-gradient(180deg, transparent, rgba(0,0,0,0.7))', borderTop: '1px solid rgba(212,168,67,0.25)' }}>
        ...
        <div className="mt-2 mx-auto px-4 py-1 rounded-full inline-block" style={{ background: 'rgba(212,168,67,0.18)', border: '1px solid rgba(212,168,67,0.35)' }}><p className="text-[10px] text-[#D4A843] font-bold tracking-wider">{municipio}</p></div>
      </div>
    ...
    <div className="relative w-full rounded-2xl overflow-hidden" style={{ aspectRatio: '3/4', background: '#ffffff', border: '3px solid #6B1D3A' }}>
      <div className="h-2/5 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #6B1D3A 0%, #9B2B55 60%, #6B1D3A 100%)' }}>
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 z-10">
          {data.foto ? (
            <div className="w-24 h-24 rounded-full overflow-hidden" style={{ border: '4px solid #D4A843', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
            ...
            <div className="w-24 h-24 rounded-full flex items-center justify-center text-3xl font-black" style={{ background: '#D4A843', border: '4px solid #D4A843', color: '#6B1D3A' }}>{nombre[0]?.toUpperCase() || '?'}</div>
          )}
        </div>
        <div className="absolute top-4 left-4 px-3 py-1 rounded-full" style={{ background: '#D4A843' }}><p className="text-[9px] font-black text-[#6B1D3A] tracking-widest uppercase">Titular Oficial</p></div>
      </div>
      <div className="h-3/5 flex flex-col items-center justify-center px-5 pt-14 pb-5">
        ...
        <p className="font-black text-5xl leading-none mb-1" style={{ color: '#6B1D3A', textShadow: '2px 2px 0px rgba(212,168,67,0.3)', fontFamily: 'Montserrat, Arial Black, sans-serif', letterSpacing: '-0.02em' }}>Esthelado</p>
        ...
        <div className="mt-3 px-4 py-1.5 rounded-full" style={{ background: '#6B1D3A' }}><p className="text-[10px] text-[#D4A843] font-bold tracking-wider uppercase">{municipio}</p></div>
...
function CardCamino({ data }: { data: PlayerData }) {
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden" style={{ aspectRatio: '3/4', background: 'linear-gradient(160deg, #F9F4EF 0%, #EDE0D4 60%, #E0CEBC 100%)', border: '2px solid #8B5E3C' }}>
      <svg className="absolute inset-0 w-full h-full opacity-[0.08]" viewBox="0 0 300 400" preserveAspectRatio="xMidYMid slice"><polygon points="0,200 60,120 120,160 180,100 240,140 300,90 300,400 0,400" fill="#6B1D3A" /><path d="M0,300 Q75,280 150,300 Q225,320 300,300 L300,400 L0,400 Z" fill="#6B1D3A" opacity="0.5" /></svg>
      <div className="relative z-10 px-5 pt-5"><p className="text-[11px] font-black tracking-[0.2em] uppercase text-center" style={{ color: '#8B5E3C', fontFamily: 'Georgia, serif' }}>Guerrero · 40 años</p><div className="w-full h-px mt-2" style={{ background: 'linear-gradient(90deg, transparent, #8B5E3C, transparent)' }} /></div>
      <div className="relative z-10 flex justify-center mt-5">
        {data.foto ? (
          <div className="w-28 h-28 overflow-hidden" style={{ borderRadius: '40% 60% 60% 40% / 55% 45% 55% 45%', border: '3px solid #8B5E3C', boxShadow: '4px 4px 0px rgba(107,29,58,0.2)' }}>
            <img src={data.foto} alt="Titular" className="w-full h-full object-cover" style={{ transform: `scale(${data.fotoScale}) rotate(${data.fotoRotate}deg)`, transformOrigin: 'center' }} />
          </div>
        ) : (
          <div className="w-28 h-28 flex items-center justify-center text-4xl font-black" style={{ borderRadius: '40% 60% 60% 40% / 55% 45% 55% 45%', background: 'rgba(107,29,58,0.15)', border: '3px solid #8B5E3C', color: '#8B5E3C' }}>{nombre[0]?.toUpperCase() || '?'}</div>
        )}
      </div>
      <div className="relative z-10 flex-1 flex flex-col items-center px-5 pt-5 pb-6">
        ...
        <p className="font-black text-[#6B1D3A] text-xl leading-tight text-center">{nombre} {apellido}</p>
        <p className="text-[11px] text-center mt-1 font-semibold" style={{ color: '#8B5E3C' }}>{municipio}</p>
        <div className="mt-auto pt-4"><div className="px-5 py-2 rounded-full border-2 text-center" style={{ borderColor: '#8B5E3C', borderStyle: 'dashed' }}><p className="text-[9px] font-black tracking-[0.25em] uppercase" style={{ color: '#8B5E3C' }}>Forjada en el territorio</p></div></div>
      </div>
    </div>
  );
}
...
export default function TarjetasPage() {
  ...
  const [playerData, setPlayerData] = useState<PlayerData>({ nombre: '', apellido: '', municipio: '', fechaNacimiento: '', foto: null, fotoScale: 1, fotoRotate: 0 });
  ...
  const updateData = (field: keyof PlayerData) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => setPlayerData(prev => ({ ...prev, [field]: e.target.value }));
  const handleGoToCard = (e: React.FormEvent) => { e.preventDefault(); if (!playerData.nombre) { alert('Ingresa al menos tu nombre.'); return; } setStep('card'); };
  ...
  const handleDownload = async () => {
    ...
    } catch { alert('Para descargar, haz una captura de pantalla de tu tarjeta.'); }
  };
  ...
  return (
    <main className="overflow-x-hidden bg-[#14050B] w-full min-h-screen">
      <div className="fixed inset-0 pointer-events-none overflow-hidden"><div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#6B1D3A]/10 rounded-full blur-[160px]" /><div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#D4A843]/5 rounded-full blur-[120px]" /></div>
      <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-8 py-10 md:py-16">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10 md:mb-14">
          <a href="/" className="inline-flex items-center gap-2 text-[#D4A843]/60 text-xs mb-6 hover:text-[#D4A843] transition-colors"><ChevronRight className="w-3 h-3 rotate-180" /> Volver al inicio</a>
          <div className="flex items-center justify-center gap-3 mb-5"><Trophy className="w-7 h-7 text-[#D4A843]" /><span className="text-[#D4A843] font-black text-lg tracking-widest uppercase">Generador Oficial</span><Trophy className="w-7 h-7 text-[#D4A843]" /></div>
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid lg:grid-cols-2 gap-8 items-start">
                <div className="rounded-3xl bg-white/[0.025] border border-white/10 p-6 md:p-8">
                  ...
                  <form onSubmit={handleGoToCard} className="space-y-4 mt-4">
                    ...
                    <div><label className={labelClass}>Municipio / Equipo *</label><select required value={playerData.municipio} onChange={updateData('municipio')} className={`${inputClass} appearance-none cursor-pointer`}><option value="" disabled className="bg-[#1A0510]">Selecciona tu municipio...</option>{MUNICIPIOS_GUERRERO.map(m => <option key={m} value={m} className="bg-[#1A0510] text-white">{m}</option>)}</select></div>
                    <div><label className={labelClass}>Tu Foto (Cámara o Galería)</label>
                      <label className="w-full flex flex-col items-center justify-center gap-2 p-6 rounded-xl border-2 border-dashed border-white/20 hover:border-[#D4A843]/40 cursor-pointer transition-colors group" htmlFor="foto-upload">
                        {playerData.foto ? <><img src={playerData.foto} alt="Preview" className="w-12 h-12 rounded-full object-cover border border-[#D4A843]/40" /><p className="text-[#D4A843] font-semibold text-sm">Foto cargada</p></> : <><Upload className="w-8 h-8 text-white/30 group-hover:text-[#D4A843]" /><p className="text-white/50 text-sm">Toma o sube una foto</p></>}
                    ...
                    {playerData.foto && (
                      <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 space-y-3">
                        ...
                        <div className="flex items-center gap-3"><ZoomOut className="w-4 h-4 text-white/40 flex-shrink-0" /><input type="range" min="0.5" max="2" step="0.1" value={playerData.fotoScale} onChange={(e) => setPlayerData(prev => ({ ...prev, fotoScale: parseFloat(e.target.value) }))} className="flex-1 accent-[#D4A843]" /><ZoomIn className="w-4 h-4 text-white/40 flex-shrink-0" /></div>
                      </div>
                    )}
                    <button type="submit" className="w-full py-4 rounded-full font-black text-base bg-[#D4A843] text-[#14050B] flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"><Star className="w-5 h-5" /> Ver mi Tarjeta Oficial</button>
                  </form>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="mt-6 p-4 rounded-2xl bg-white/[0.02] border border-white/10 flex flex-col items-center">
                    ...
                    <div className="w-[180px] h-[180px]">{selectedModel === 'mundialista' && <CardMundialista data={playerData} />}{selectedModel === 'esthelado' && <CardEsthelado data={playerData} />}{selectedModel === 'camino' && <CardCamino data={playerData} />}</div>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {step === 'card' && (
            <motion.div key="card" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md mx-auto">
              <div className="text-center mb-8"><p className="text-[#D4A843] text-sm font-semibold uppercase tracking-widest mb-2">{models.find(m => m.id === selectedModel)?.emoji} Tu tarjeta oficial está lista</p><h2 className="text-white font-black text-2xl">{playerData.nombre} {playerData.apellido}</h2></div>
              <div ref={cardRef} className="mx-auto w-[360px] h-[360px]">{selectedModel === 'mundialista' && <CardMundialista data={playerData} />}{selectedModel === 'esthelado' && <CardEsthelado data={playerData} />}{selectedModel === 'camino' && <CardCamino data={playerData} />}</div>
              <div className="mt-6 flex gap-2 justify-center">{models.map(m => (<button key={m.id} onClick={() => setSelectedModel(m.id)} className="px-3 py-2 rounded-xl text-sm font-semibold transition-all" style={{ background: selectedModel === m.id ? 'rgba(212,168,67,0.2)' : 'rgba(255,255,255,0.04)', border: `1px solid ${selectedModel === m.id ? '#D4A843' : 'rgba(255,255,255,0.1)'}`, color: selectedModel === m.id ? '#D4A843' : 'rgba(255,255,255,0.4)' }}>{m.emoji}</button>))}</div>
              <div className="mt-8 space-y-3">
                <button onClick={handleDownload} className="w-full py-4 rounded-full font-black flex items-center justify-center gap-2 transition-all hover:scale-[1.02] bg-[#D4A843] text-[#14050B]">{downloaded ? <Check className="w-5 h-5" /> : <Download className="w-5 h-5" />}{downloaded ? '¡Descargada!' : 'Descargar 1080x1080'}</button>
                <button onClick={handleShareWhatsApp} className="w-full py-4 rounded-full font-bold flex items-center justify-center gap-2 bg-[#6B1D3A] border border-[#D4A843]/40 text-white transition-all hover:scale-[1.02]"><Share2 className="w-5 h-5 text-[#D4A843]" /> Compartir en WhatsApp</button>
